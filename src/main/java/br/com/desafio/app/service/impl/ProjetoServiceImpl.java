...
import br.com.desafio.app.domain.*;
...
public class ProjetoServiceImpl implements ProjetoService {
    ...
    private static final long PRAZO_BAIXO_DIAS = 90;   
    private static final long PRAZO_ALTO_DIAS = 180;   
    ...
    public void excluir(Long id) {
        ...
        if (STATUS_BLOQUEIAM_EXCLUSAO.contains(projeto.getStatusAtual())) {
            throw new RegraNegocioException(
                    "Não é permitido excluir projeto com status iniciado, em andamento ou encerrado"
            );
        }
    ...
    private ClassificacaoRisco calcularRisco(Projeto projeto) {
        ...
        long prazoDias = calcularPrazoDias(
        ...
        if (orcamento.compareTo(LIMITE_ALTO) > 0 || prazoDias > PRAZO_ALTO_DIAS) {
            return ClassificacaoRisco.ALTO;
        }

       
        if (orcamento.compareTo(LIMITE_BAIXO) <= 0 && prazoDias <= PRAZO_BAIXO_DIAS) {
    ...
    private long calcularPrazoDias(LocalDate inicio, LocalDate fim) {
        return Math.max(ChronoUnit.DAYS.between(inicio, fim), 0);
    }
    ...
    private Set<Membro> buscarMembros(Set<Long> membrosIds) {
        var membros = membroRepository.findAllById(membrosIds)
                .stream()
    ...
    private void validarMembros(Set<Membro> membros, Long projetoId) {
        ...
        for (Membro membro : membros) {
            if (membro.getAtribuicao() != AtribuicaoMembro.FUNCIONARIO) {
                throw new RegraNegocioException(
                        "Apenas membros FUNCIONARIO podem ser associados ao projeto"
                );
            }
            ...
            if (ativos >= 3) {
                throw new RegraNegocioException(
                        "Membro " + membro.getNome() + " já está alocado em 3 projetos ativos"
                );
            }
        }
    }
    ...
    private void preencherProjeto(Projeto projeto,
        ...
        StatusProjeto novoStatus =
                dto.statusAtual() != null ? dto.statusAtual() : StatusProjeto.EM_ANALISE;
    ...
    private void validarDatas(ProjetoRequestDTO dto) {
        if (dto.previsaoTermino().isBefore(dto.dataInicio())) {
            throw new RegraNegocioException(
                    "Previsão de término deve ser maior ou igual à data de início"
            );
        }

        if (dto.dataRealTermino() != null &&
                dto.dataRealTermino().isBefore(dto.dataInicio())) {
            throw new RegraNegocioException(
                    "Data real de término deve ser maior ou igual à data de início"
            );
        }
    }
}